...
func backtrace(_ thread: thread_t, stack: UnsafeMutablePointer<UnsafeMutableRawPointer?>!, _ maxSymbols: Int32) -> Int32

public class RCBacktrace {

    private static var main_thread_t: mach_port_t?

    /// should call this method in main thread first
    public static func setup() {
        main_thread_t = main_thread_t ?? mach_thread_self()
    }

    public static func callstack(_ thread: Thread) -> [StackSymbol] {
        let mthread = machThread(from: thread)
        return mach_callstack(mthread)
    }

    public static func mach_callstack(_ thread: thread_t) -> [StackSymbol] {
